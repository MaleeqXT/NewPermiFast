...
// ── Welcome tab ───────────────────────────────────────────────────────────────
function TabAccueil({ studentName, balance, usedBalance, canViewContract, onOpenContract, onOpenNotifications, onOpenCommander, onOpenCarnet }) {
...
function TabSessions({ studentId }) {
  return <StudentAvailableSessions studentId={studentId} />;
}

// ── Profile tab ───────────────────────────────────────────────────────────────
function TabProfile({ studentName, studentEmail, onOpenStudentProfile, onOpenExamInformation, onOpenCommander, onOpenOrders, onOpenCancellations }) {
  const OPTIONS = [
    { label: "Infos examen",
...
export default function StudentDashboard({ onClose, openFilterOnOpen = false }) {
  ...
  return (
    <DashboardShell {...shellProps}>
      {activeTab==="accueil"  && <TabAccueil studentName={studentName} balance={studentBalance} usedBalance={usedBalance} canViewContract={canViewContract} onOpenContract={handleOpenContract} onOpenNotifications={() => setShowNotifications(true)} onOpenCommander={() => setShowCommander(true)} onOpenCarnet={() => setShowCompetences(true)} />}
      {activeTab==="sessions" && <TabSessions studentId={viewedStudentId} />}
      {activeTab==="profile"  && (
        <TabProfile
